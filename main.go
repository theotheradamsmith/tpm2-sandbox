...
func credentialActivation() error {
	akNameData, err := ioutil.ReadFile("ak.name")
	if err != nil {
		log.Println("Unable to open ak.name")
		return err
	}
	akPubBlob, err := ioutil.ReadFile("ak.pub.blob")
	if err != nil {
...
func caChallenge(nameData []byte, pubBlob []byte) ([]byte, []byte, error) {
	...
	*/
	...
	ekPubDer, err := ioutil.ReadFile("ek.pub")
	if err != nil {
...
func generateAK() error {
	...
	if err := ioutil.WriteFile("ak.name", nameData, 0644); err != nil {
		log.Println("Failed to write ak.name")
		return err
	}
	if err := ioutil.WriteFile("ak.pub.blob", pubBlob, 0644); err != nil {
